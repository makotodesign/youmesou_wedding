...
class icit_srdb_ui extends icit_srdb {
	...
	public function response( $name = '', $user = '', $pass = '', $host = '127.0.0.1', $port = 3306, $charset = 'utf8', $collate = '' ) {
		...
		$exclude_cols = isset( $_POST[ 'exclude_cols' ] ) ? $_POST[ 'exclude_cols' ] : [];
		$include_cols = isset( $_POST[ 'include_cols' ] ) ? $_POST[ 'include_cols' ] : [];
	...
	public function ui() {
		...
		if ( ini_get( 'safe_mode' ) ) {
			...
			<div class="special-errors">
				<h4>Warning</h4>
				<?= printf( '<p>Safe mode is on so you may run into problems if it takes longer than %s seconds to process your request.</p>', ini_get( 'max_execution_time' ) ); ?>
			</div>
		<?php
		}
