...
class OoCsvImportController extends AbstractCsvImportController
{
	/**
...
/* oo_customize-- */
						...
						if (isset($row[$headerByKey['input_process']])) {
							if (StringUtil::isNotBlank($row[$headerByKey['input_process']])) {
								$Product->setInputProcess(StringUtil::trimAll($row[$headerByKey['input_process']]));
							} else {
								$Product->setInputProcess(null);
							}
						}

						if (isset($row[$headerByKey['wp_products_code']])) {
							if (StringUtil::isNotBlank($row[$headerByKey['wp_products_code']])) {
								$Product->setWpProductsCode(StringUtil::trimAll($row[$headerByKey['wp_products_code']]));
							} else {
								$Product->setWpProductsCode(null);
							}
						}
/* --oo_customize */
...
/* oo_customize-- */
			trans('admin.product.product_csv.input_process_col') => [
				'id' => 'input_process',
				'description' => 'admin.product.product_csv.input_process_description',
				'required' => false,
			],
			trans('admin.product.product_csv.wp_products_code_col') => [
				'id' => 'wp_products_code',
				'description' => 'admin.product.product_csv.wp_products_code_description',
				'required' => false,
			],
/* --oo_customize */
		];
